...
/*
 * src/include/plugin/rma/rma_v14.h — RMA 插件 v14 接口 [NVIDIA 插件接口/第三方]
 * ----------------------------------------------------------------------------
 * 定义 RMA(远程内存访问)插件的 v14 版本接口（配置与上下文结构）。
 */
...
typedef struct {
  ...
  // 创建 a 设置 of 连接 在 ... 之间 组. Config indicates 连接 properties,
  // like 的数量 上下文 并且 traffic 类.
  ...
  // Progress 函数. 将会 被调用 若 non-NULL.
  ncclResult_t (*rmaProgress)(void* rmaCtx);

  // Query 最后一个 错误 为了 RMA 支持. Particularly important 当 rmaProgress is 不 已使用, to 报告 错误.
