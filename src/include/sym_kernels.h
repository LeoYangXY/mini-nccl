...
/*
 * include/sym_kernels.h — symmetric(对称) kernel 相关声明
 * ----------------------------------------------------------------------------
 * 声明 NCCL 的“对称内存(symmetric memory)”kernel 接口：在多个 rank 上以相同布局
 * 分配内存并协作执行 kernel（用于 AllGatherV 等需要对称视图的集合）。
 */
...
////////////////////////////////////////////////////////////////////////////////
// ncclSymk[Foo]: 内核 已构建 on 该设备 API
...
struct alignas(16) ncclSymkDevWorkArgs {
  ...
  int nMaxChannels;
  int maxDynamicSmem;
  // starting of channelWorkRange 将会 已对齐 to 16 字节
...
// We 假设 ncclComm contains a 字段: `ncclSymkState symkState`
...
// Generated by 源/设备/symmetric/generate.py
