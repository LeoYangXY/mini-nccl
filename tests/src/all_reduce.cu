...
// mini-nccl: 本仓库只验证标准 ncclAllReduce API, 不需要依赖 DOCA 的设备内核(LSA/multimem)。
// nccl_device.h 会引入 doca_gpunetio 头(本机未安装)导致编译失败, 这里强制把 NCCL_VERSION_CODE
// 视为 2.27, 使所有 `#if NCCL_VERSION_CODE >= 2.28` 的设备内核代码被跳过, 仅保留 case 0 标准 API。
