 ...
 ************************************************************************/
/*
 * tests/src/common.h — 测试公共头文件
 * ----------------------------------------------------------------------------
 * 定义各性能测试共用的配置与辅助：数据规模/数据类型/迭代次数等参数、NCCL 版本
 * 强制（本仓库强制 NCCL_VERSION_CODE=22700 以跳过 DOCA 设备内核，仅保留标准 API）、
 * 计时与校验宏。被所有 *_perf 测试包含。
 */
...
#include "nccl.h"
// mini-nccl: 本仓库只验证标准 ncclAllReduce API, 不需要 DOCA 设备内核。
// nccl_device.h 会引入 doca_gpunetio 头(本机未安装), 这里强制把 NCCL_VERSION_CODE 视为 2.27,
// 使所有 `#if NCCL_VERSION_CODE >= 2.28` 的设备内核代码被跳过, 仅保留 case 0 标准 API。
#ifdef NCCL_VERSION_CODE
#undef NCCL_VERSION_CODE
#endif
#define NCCL_VERSION_CODE 22700
