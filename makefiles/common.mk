...
ifeq ($(NCCL_OS_LINUX), 1)
  # mini-nccl: GIN (GPU-initiated networking) removed; single-node build
  CXXFLAGS += -DNCCL_GIN_PROXY_ENABLE=1
  CXXFLAGS += -DNCCL_GIN_GDAKI_ENABLE=0
  NVCUFLAGS += -DNCCL_GIN_PROXY_ENABLE=1
...
ifeq ($(NCCL_OS_LINUX), 1)
  ...
  # mini-nccl: GIN removed (must be after NVCUFLAGS := above)
  NVCUFLAGS += -DNCCL_GIN_PROXY_ENABLE=1 -DNCCL_GIN_GDAKI_ENABLE=0
else ifeq ($(NCCL_OS_WINDOWS), 1)
